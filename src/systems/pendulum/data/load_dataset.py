...
def load_pendulum(model_type="hnn"):
    ...
    chemin_data_train = data_dir / 'dataset_train.npz'
    chemin_data_test = data_dir / 'dataset_test.npz'
    
    if not chemin_data_train.exists():
        raise FileNotFoundError(f"Fichier non trouvé: {chemin_data_train}")
    if not chemin_data_test.exists():
        raise FileNotFoundError(f"Fichier non trouvé: {chemin_data_test}")
